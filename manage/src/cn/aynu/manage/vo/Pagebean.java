...
public class Pagebean {
	...
	public String toString() {
		return "Pagebean [list=" + list + ", allrow=" + allrow + ", allpage=" + allpage + ", currentpage=" + currentpage
				+ ", pagesize=" + pagesize + ", isfirstpage=" + isfirstpage + ", islastpage=" + islastpage
				+ ", hasqianpage=" + hasqianpage + ", hasnextpage=" + hasnextpage + "]";
	}
	...
	// 得到总页数
	public static int allPage(final int pagesize, final int allrow) {// 设问号之前的为条件，如果该条件为真，则返回分号之前的值，否则返回分号之后的值
		int allpage = allrow % pagesize == 0 ? allrow / pagesize : allrow / pagesize + 1;
		return allpage;
	}

	// 得到当前页第一条在数据库中的位置
	public static int jilushu(final int pagesize, final int currentpage) {
